...
<div
	class="item-card group block py-6 -mx-4 px-4 rounded-sm hover:bg-neutral-50 dark:hover:bg-neutral-900/50 transition-colors duration-300 cursor-pointer"
	role="link"
	tabindex={0}
	onclick={handleCardClick}
	onkeydown={(e) => e.key === 'Enter' && goto(path)}
>
		...
		<div class="ml-6 md:ml-8 lg:ml-10 flex flex-col grow min-w-0">
			<div class="flex flex-col md:flex-row md:items-baseline md:justify-between gap-1 md:gap-4">
				<h3 class="text-lg md:text-xl font-medium text-neutral-900 dark:text-neutral-100 tracking-tight group-hover:text-neutral-700 dark:group-hover:text-neutral-200 transition-colors duration-300">
					<a href={path} class="hover:no-underline">{title}</a>
				</h3>
			...
			<div class="flex flex-row flex-wrap gap-4 mt-auto pt-3 text-sm font-light text-neutral-400 dark:text-neutral-500">
				{#if slides}
					<span class="hidden md:inline hover:text-neutral-700 dark:hover:text-neutral-300 transition-colors duration-200"><a href={slides} target="_blank">Slides</a></span>
				{/if}
				{#if additionalLinks}
					{#each additionalLinks as { title: linkTitle, url }, i (url)}
						<span class="hidden md:inline hover:text-neutral-700 dark:hover:text-neutral-300 transition-colors duration-200"><a href={url} target="_blank">{linkTitle}</a></span>
					{/each}
				{/if}
